...
class PaymentFileHeaderResourceIT {
    ...
    @Mock
    private PaymentFileHeaderRepository paymentFileHeaderRepositoryMock;

    @Autowired
    private PaymentFileHeaderMapper paymentFileHeaderMapper;

    @Mock
    private PaymentFileHeaderService paymentFileHeaderServiceMock;
    ...
    public static PaymentFileHeader createEntity(EntityManager em) {
        ...
        paymentFileHeader.setProject(project);
    ...
    void getAllPaymentFileHeadersWithEagerRelationshipsIsEnabled() throws Exception {
        when(paymentFileHeaderServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restPaymentFileHeaderMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(paymentFileHeaderServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllPaymentFileHeadersWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(paymentFileHeaderServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
