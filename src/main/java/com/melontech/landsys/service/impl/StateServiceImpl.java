...
public class StateServiceImpl implements StateService {
    ...
    /**
     *  Get all the states where Land is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<StateDTO> findAllWhereLandIsNull() {
        log.debug("Request to get all states where Land is null");
        return StreamSupport
            .stream(stateRepository.findAll().spliterator(), false)
            .filter(state -> state.getLand() == null)
            .map(stateMapper::toDto)
