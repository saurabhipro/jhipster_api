...
public class KhatedarServiceImpl implements KhatedarService {
     ...
     */
    @Transactional(readOnly = true)
    public List<KhatedarDTO> findAllWherePaymentFileIsNull() {
        log.debug("Request to get all khatedars where PaymentFile is null");
        return StreamSupport
            .stream(khatedarRepository.findAll().spliterator(), false)
            .filter(khatedar -> khatedar.getPaymentFile() == null)
            .map(khatedarMapper::toDto)
    ...
    public Optional<KhatedarDTO> findOne(Long id) {
        ...
        return khatedarRepository.findById(id).map(khatedarMapper::toDto);
    }
